...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // glm::mat4 model = glm::mat4(1.0f); // 단위 행렬(identity matrix)로 초기 설정
        glm::mat4 view = glm::mat4(1.0f);
        glm::mat4 projection;
        // model = glm::rotate(model, (float)glfwGetTime() * glm::radians(-55.0f), glm::vec3(0.5f, 1.0f, 0.0f)); // x, y, z축을 기준으로 회전
        view = glm::translate(view, glm::vec3(0.0f, 0.0f, -3.0f)); // z축을 따라 -3만큼 이동
        projection = glm::perspective(glm::radians(45.0f), 800.0f / 600.0f, 0.1f, 100.0f);
        // matrix uniform에 값 할당
